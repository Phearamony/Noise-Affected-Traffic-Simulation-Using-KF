...
hold on;
CLR1=[1 0 1; 0.5 1 1; 0 0.5 0.5; 1 0.5 0.25];
if(length(plt)>2)
    delete(plt(2:end));
end
...
for J=1:length (xr)
    plt(end+1)= plot(xr(J), yr(J), 'sk','LineWidth',.151, 'MarkerSize', 4.5,'MarkerFaceColor', CLR1(4,:)); %, 'MarkerFacColor',
    plt(end+1)=  plot(xr(J)-2.5, yr(J), 'sk','LineWidth',.151, 'MarkerSize', 5.5,'MarkerFaceColor', CLR1(4,:)); %, 'MarkerFacColor',
end
